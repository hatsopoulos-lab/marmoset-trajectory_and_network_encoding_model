...
new_tag = '_encoding_model_regularized_results_30ms_shift_v3'  
pkl_outfile = base + 'DM' + new_tag + '.pkl'

debugging = False
skip_data_sampling = True
run_model_only = True
...
def create_model_features_and_store_in_dict(nComps):
    ...
    for lead, lag in zip(params.lead, params.lag):
        ...
        traj_pca_features_pos       = np.hstack((scaled_traj_pca_features, scaled_avg_pos_samples[idx]))
        # traj_pca_features_speed     = np.hstack((traj_pca_features, np.expand_dims(scaled_avg_speed_samples[idx], axis=1)))
        # traj_pca_features_pos_and_speed = np.hstack((traj_pca_features, np.expand_dims(avg_speed_samples[idx], axis=1), avg_pos_samples[idx]))
        # short_traj_pca_features_pos = np.hstack((short_traj_pca_features, scaled_short_avg_pos_samples[idx]))
        
        traj_and_avgPos_features = np.hstack((traj_features, scaled_avg_pos_samples[idx]))
...
def run_models():
    for lead, lag in zip(params.lead, params.lag):    
        ...
        model_keys.append('%s_shuffled_spike_samples' % params.shuffle_to_test)
        ...
        for model in model_keys:
            if 'shuffle' in model:
                ...
                alpha, l1 = choose_regularization_params(params.shuffle_to_test, choose_by = params.average_reg_choice_over)
            else:
                traj_features = results_dict[lead_lag_key]['model_features'][model]
                alpha, l1 = choose_regularization_params(model, choose_by = params.average_reg_choice_over)
            ...
            model_results[model] = train_and_test_glm(tmp_traj_features, tmp_spike_samples, model, RNGs, lead_lag_key, alpha, l1)
...
def test_regularization_params_over_lead_lags():
