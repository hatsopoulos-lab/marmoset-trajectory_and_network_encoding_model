...
if marmcode=='TY':
    ...
    modulation_base = '/project/nicho/projects/dalton/data/TY/TY20210211_freeAndMoths-003_resorted_20230612_DM'
    ...
    modulation_base = '/project/nicho/projects/dalton/data/MG/MG20230416_1505_mothsAndFree-002_processed_DM'
...
class params:
    ...
    if marmcode == 'TY':
        ...
        cortical_boundaries = {'x_coord'      : [         0,       400,       800,      1200,    1600,    2000,    2400,    2800,    3200,    3600],
                                ...
                                'areas'        : ['Sensory', 'Sensory', 'Sensory', 'Sensory', 'Motor', 'Motor', 'Motor', 'Motor', 'Motor', 'Motor'],
...
def plot_result_on_channel_map(units_res, jitter_radius = .15, hue_key='W_in', 
    ...
    try:
        title = [f'{lab} AUC' for lab, key in zip(['Trajectory', 'Full Kinematics', 'Velocity', 'Short Kinematics', 'Kinematics + reachFN', 'Kinematics + spontaneousFN \nGeneralization'], 
    ...
    sns.scatterplot(ax = ax, data = scatter_units_res, x = 'scatter_x', y = 'scatter_y', 
                    ...
                    palette=palette, sizes = sizes, s = s)
    ...
    fig.savefig(os.path.join(fig_base, f'{hue_key}_on_array_map.png'), bbox_inches='tight', dpi=plot_params.dpi)    
...
def sort_units_by_column_for_FN_plots(units_res):
    ...
    labels = []
    units_sorted = pd.DataFrame() 
    # for x in np.unique(units_res['x']): 
    #     tmp_df = units_res.copy().loc[units_res['x']==x, :]
    #     tmp_df.sort_values(by='W_in', inplace=True)
    #     units_sorted = pd.concat((units_sorted, tmp_df), axis=0, ignore_index=False)
    #     if tmp_df.size > 0:
    #         col_idxs = np.where(units_sorted['x']==x)
    #         ticks.append (np.max(col_idxs)+1)
    #         labels.append('')
...
def evaluate_effect_of_network_shuffles(lead_lag_key, comparison_model, 
    ...
    for mode in train_auc_df['mode'].unique(): 
        for idx, percent in enumerate(train_auc_df['percent'].unique()):    
            ...
            print(f'fullShuffle > partialShuffle, {percent}%, {mode}:  p={np.round(sign_test.pvalue, 4)}, nStrength={sign_test.k}, nUnits={sign_test.n}, prop={np.round(sign_test.proportion_estimate, 2)}')

            if sign_test.proportion_estimate > 0.77:
                sig_strength_v_fullShuffle[mode].append(idx)
    ...
    for metric in auc_df.columns:
        if 'mod' in metric or 'dev' in metric or 'frate' in metric: 
            ...
            sns.kdeplot(ax=ax, data=tmp_df, linewidth=plot_params.distplot_linewidth,
                        x=metric, common_norm=False, bw_adjust=0.4, cumulative=False, color=subset_colors[full_index])
    ...
    for sub_basis, FN_set in product(subset_basis, FN_sets):
        ...
        for FN, FN_name in zip(FN_tmp, FN_names):
            ...
            tmp_df = pd.DataFrame(data=zip(sub_FN.flatten(), 
                                           ...
                                           sub_average_auc.flatten(),
                                           sub_dist.flatten()), 
                                  columns=['Wji', 'input_area', 'FN_key', 'Units Subset', 
...
def find_reach_specific_group(diff_df, model_1, model_2, paperFig = 'FigS5'):
    ...
    for unit_model1, unit_model2 in zip(full_res_model_1, full_res_model_2):        
        ...
        #print(f'model2 > model1:  p={np.round(sign_test.pvalue, 4)}, nStrength={sign_test.k}, nUnits={sign_test.n}, prop={np.round(sign_test.proportion_estimate, 2)}')
    ...
    lastUnit = np.where(medFilt_grad < 0.15 * medFilt_grad.max())[0][0]
    # lastUnit = np.where(medFilt_grad < 0.05 * sorted_diff_df['dist_positive_grad'].max())[0][0]-1
    ...
    ax.plot(np.arange(diff_df.shape[0]), sorted_diff_df['dist_positive_grad']*2, linewidth = plot_params.distplot_linewidth)
    ax.plot(np.arange(diff_df.shape[0]), medFilt_grad*2, linewidth = plot_params.distplot_linewidth)
    ...
    ax.legend(labels=['AUC Difference', 'd/du(AUC Difference)', 'Median Filter of Derivative'],
    ...
    # reach_specific_units_byStats = diff_df.index[diff_df['generalization_proportion'] >= 0.925] 
    # non_specific_units_byStats   = diff_df.index[diff_df['generalization_proportion'] < 0.925]
...
if __name__ == "__main__":
    ...
    for model_x, model_y, fignum in zip(model_list_x, model_list_y, fig_list):    
        ...
        if model_y not in plotted_on_map_record:
            plot_result_on_channel_map(units_res, jitter_radius = .2, hue_key='W_in', 
                                       size_key = model_y,  title_type = 'size', style_key=None,
    ...
    plot_result_on_channel_map(units_res, jitter_radius = .2, hue_key='Units Subset', 
                               size_key = None,  title_type = 'hue', style_key=None,
