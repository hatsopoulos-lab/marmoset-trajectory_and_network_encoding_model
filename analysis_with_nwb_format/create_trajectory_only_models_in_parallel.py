...
tmp_job_array_folder = base + '_tmp_job_array_files'

base, old_tag = base.split('DM')
# new_tag = '_analyze_trajectory_only_models'  
new_tag = '_encoding_model_sorting_corrected_30ms_shift_v2'
pkl_infile = base + 'DM' + new_tag + '.pkl'

debugging=False
redo_feature_creation=False

use_traj_regParams_for_traj_avgPos = True
...
class params:
    ...
    shuffle_to_test = ['traj_avgPos', 'position']
    minSpikeRatio = .005
    nDims = 3
    frate_thresh = 2
    snr_thresh = 3
...
def add_tmp_files_to_pkl():   
    ...
    for job_file in job_array_files:
        with open(job_file, 'rb') as f:
            model_results = dill.load(f)
            
        lead_lag_key = model_results['lead_lag_key']
        del model_results['lead_lag_key'] 
...
def run_models():
    ...
    lead_lag_key  = lead_lag_keys[task_id] 
    ...
    model_keys.extend(['%s_shuffled_spike_samples' % shuf_model for shuf_model in params.shuffle_to_test])
    ...
    model_results['lead_lag_key'] = lead_lag_key

    for model in model_keys:
        features_key = model.split('_shuffled')[0]
    ...
    with open(pkl_tmp_job_file, 'wb') as f:
        dill.dump(model_results, f, recurse=True)
...
if __name__ == "__main__":
    ...
    if n_tasks != len(results_dict.keys()) and not debugging:
