...
if marmcode=='TY':
    ...
    extra_tag = 'cortical_networks'#'_FINAL_TEST'
elif marmcode=='MG':
    nwb_infile = ''
    pkl_infile = ''
    extra_tag = ''
...
class params:
    ...
    lead_lag_keys_for_network = ['lead_200_lag_200', 'lead_200_lag_300']
...
def add_tmp_files_to_pkl():   
    ...
    for job_file in job_array_files:
        ...
        # if model_name not in results_dict[lead_lag_key]['model_results'].keys():
            # results_dict[lead_lag_key]['model_results'][model_name] = model_results
        # else:
        #     tmp_auc   = results_dict[lead_lag_key]['model_results'][model_name]['AUC']
        #     tmp_coefs = results_dict[lead_lag_key]['model_results'][model_name]['coefs']

        #     tmp_auc[:  , sample_range.start:sample_range.stop] = model_results[  'AUC'][:  , sample_range.start:sample_range.stop]
        #     tmp_coefs[..., sample_range.start:sample_range.stop] = model_results['coefs'][..., sample_range.start:sample_range.stop]

        #     results_dict[lead_lag_key]['model_results'][model_name]['AUC'] = tmp_auc
        #     results_dict[lead_lag_key]['model_results'][model_name]['coefs'] = tmp_coefs
