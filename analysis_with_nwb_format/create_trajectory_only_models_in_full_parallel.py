...
if marmcode=='TY':
    ...
    new_tag = '_trajectory_shuffled_tortuosity_split_encoding_models_30ms_shift_v1'
elif marmcode=='MG':
    nwb_infile   = '/project/nicho/projects/dalton/data/MG/MG20230416_1505_mothsAndFree-002_processed_DM.nwb'
    new_tag = '_traj_TEST_trajectory_shuffled_encoding_models_30ms_shift_v1'
    
base, ext = os.path.splitext(nwb_infile)

base, old_tag = base.split('DM')
# new_tag = '_traj_avgPos_TEST_spikes_shuffled_encoding_models_30ms_shift_v1'  
...
class params:
    ...
    num_models_including_shuffles = 14
    num_model_iters = 100
    if debugging:
    ...
    for samp in iter_range:
        ...
        for unit, (trainSpks, trainFts, testFts, testSpks, shuf_rng) in enumerate(zip(trainSpikes, trainFeatures, testFeatures, testSpikes, RNGs['spike_shuffle'])):
            ...
            if 'shuffle' in model_key:
                if params.feature_to_shuffle == 'spikes':
                    trainSpks = shuf_rng.permutation(trainSpks)
                elif params.feature_to_shuffle == 'traj':
        ...
        for unit, preds in enumerate(predictions):
            ...
            areaUnderROC[unit, samp] = auc(falsePosProb, hitProb)
...
if __name__ == "__main__":
    ...
    if n_tasks != n_tasks_needed and not debugging:
        ...
        tmp_job_array_folder = '%s_v%s_tmp_job_array_files' % (base, out_version)
        ...
        if task_id == n_tasks-1:
            while len(glob.glob(os.path.join(tmp_job_array_folder, '*'))) < n_tasks:
                print('waiting for all jobs to finish model creation', flush=True)
                time.sleep(10)
