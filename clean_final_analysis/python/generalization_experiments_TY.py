...
pkl_out_tag = 'generalization_experiments' 
...
tmp_job_array_folder = hdf_outfile.parent / 'network_jobs_tmp_files' / f'{hdf_outfile.stem}'          
models_storage_folder = Path('/scratch/midway3/daltonm/analysis/stored_encoding_models') / hdf_outfile.stem
...
def add_tmp_files_to_hdf():   
    ...
    save_dict_to_hdf5(results_dict, hdf_outfile)
...
if __name__ == "__main__":
    ...
    models_dict = {'full_vs_spont_FN' : {'model_names'           : [     'kin_model_spont_train_reach_test_FN',     
                                                                          'kin_model_rest_train_reach_test_FN', 
                                                                       'kin_model_groomed_train_reach_test_FN', 
                                                                    'kin_model_locomotion_train_reach_test_FN', 
                                                                      'kin_model_climbing_train_reach_test_FN',
                                                                       'kin_model_armMove_train_reach_test_FN'
                                                                   ],  
                                         'shuf_mode'             : 'unaltered',
                                         'models_per_set'        : 1,
                                         'FN'                    : [spontaneous_FN.copy(), 
                                                                    annotated_FN_dict['Rest'].copy(), 
                                                                    annotated_FN_dict['Groomed'].copy(), 
                                                                    annotated_FN_dict['Locomation'].copy(),
                                                                    annotated_FN_dict['Climbing'].copy(),
                                                                    annotated_FN_dict['Directed_arm_movement'].copy()],
                                         'FN_key'                : ['spontaneous_FN', 'rest', 'groomed', 'locomotion', 'climbing', 'arm_movements'],
                                         'test_FN'               : [reach_FN.copy(),
                                                                    reach_FN.copy(),
                                                                    reach_FN.copy(),
                                                                    reach_FN.copy(),
                                                                    reach_FN.copy(),
                                                                    reach_FN.copy()],
                                         'test_FN_key'           : ['reach_FN', 'reach_FN', 'reach_FN', 'reach_FN', 'reach_FN', 'reach_FN'],
                                         'trained_glm_source'    : [None, None, None, None, None, None],
                                         'save_GLMs'             : [False, False, False, False, False, False],
                                         'save_network_features' : [False, False, False, False, False, False]},
                   }
    ...
    for model_set, task_info in enumerate(task_model_list):
        ...
        pkl_tmp_job_file = tmp_job_array_folder / f'{hdf_outfile.stem}_tmp_job_{str(task_id).zfill(3)}_model_set_{str(model_set).zfill(2)}.pkl'
