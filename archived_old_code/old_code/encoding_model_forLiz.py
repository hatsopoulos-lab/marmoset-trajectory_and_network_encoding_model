...
class path:
    storage = r'C:/Users/daltonm/Documents/Lab_Files/updated_trajectory_analysis/trajectories_code_and_figures/DLC_validation/'
    
class params:
    spkSampWin = 0.01 # sample in 10 ms bins
    trajShift = 0.02 #sample every 20ms
    ...
    for sessIdx in range(0, len(allData.camSignals['eventBoundaries']), nCams):#[8]:  # to pull only session 3 data. change the this for future: #range(0, len(allData.camSignals['eventBoundaries']), nCams):
        ...
        for evt in range(np.shape(boundaries)[-1]):
            ...
            if len(eventExp) == trajLength:
                ...
                ################################
                #+++ Here's where I extract trajectories
